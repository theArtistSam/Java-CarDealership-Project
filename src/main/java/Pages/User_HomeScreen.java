...
public class User_HomeScreen {
    ...
    // Side Buttons
    private Button Dashboard, Store, Booking, Logout, Download;
...
//
        ...
        if(isReport && array.size() != 0){
            ...
            try{
                ...
                PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("E:\\School Projects\\DB Projects\\DB Mongo\\src\\main\\Reports\\Customer\\"+Name+"_Purchase_Report.pdf"));
                document.open();
                ...
                document.add(new Paragraph("User Name: " + Name + "              " + "Today's Date: "  + (cD[2] + "-" + cD[1] + "-" + cD[0]) + "\n\n"));
                document.add(table);
                document.add(new Paragraph("\nTotal Purchase Price: " + (t_price + "")));

                document.close();
                writer.close();


                File report_file = new File("E:\\School Projects\\DB Projects\\DB Mongo\\src\\main\\Reports\\Customer\\"+Name+"_Purchase_Report.pdf");
            ...
            JOptionPane.showMessageDialog(this.frame, "No Cars Purchased Yet!");
        }
    }
